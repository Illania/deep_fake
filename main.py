...
async def upload_video(uploaded_file: UploadFile = File(...)):
    """Uploads destination video to the server VIDEOS_FOLDER using form data."""
    try:
        clean_folder(VIDEOS_FOLDER)
        return await upload_to_folder(uploaded_file, SourceType.VIDEO, VIDEOS_FOLDER)
...
async def upload_image(uploaded_file: UploadFile = File(...)):
    """Uploads destination face image to the server IMAGES_FOLDER using form data."""
    try:
        clean_folder(IMAGES_FOLDER)
        return await upload_to_folder(uploaded_file, SourceType.IMAGE, IMAGES_FOLDER)
    except Exception as e:
        return {"message": f"There was an error uploading the file:{str(e)}"}
    finally:
        uploaded_file.file.close()
...
async def upload_multispecific(uploaded_file: UploadFile = File(...)):
    """Uploads archived multispecific folder to the server MULTISPECIFIC_FOLDER using form data."""
    try:
        clean_folder(MULTISPECIFIC_FOLDER)
        return await upload_to_folder(uploaded_file, SourceType.ARCHIVE, MULTISPECIFIC_FOLDER)
    except Exception as e:
        return {"message": f"There was an error uploading the file:{str(e)}"}
    finally:
        uploaded_file.file.close()
